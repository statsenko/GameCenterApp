...
public class GameCenterExample : MonoBehaviour {
	...
	void OnGUI() {
		...
		if(GUI.Button(new Rect(330, 250, 150, 50), "Show Leader Board2 Today")) {
			GameCenterManager.showLeaderBoard(leaderBoardId2, GCBoardTimeSpan.TODAY);
		}


		if(GUI.Button(new Rect(330, 310, 150, 50), "Report Score LB2")) {
			hiScore++;
			GameCenterManager.reportScore(hiScore, leaderBoardId2);
		}

		if(GUI.Button(new Rect(330, 370, 150, 50), "Get Score LB 2")) {
			GameCenterManager.getScore(leaderBoardId2);
		}


		if(GUI.Button(new Rect(330, 430, 150, 50), "Show Leaderboards")) {
