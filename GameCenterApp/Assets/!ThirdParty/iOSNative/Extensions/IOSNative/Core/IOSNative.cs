...
public class IOSNative : EventDispatcher {
	...
	public const string APPLE_ID = "880714740"; // REPLACE_WITH_YOUT_APPLE_ID
	...
	void Start() {
		...
		if(APPLE_ID.Equals("REPLACE_WITH_YOUT_APPLE_ID")) { // 
			Debug.LogError ("Apple ID is empty");
		}

		if(Application.platform == RuntimePlatform.IPhonePlayer && !_IsInited) {
			Debug.Log("IOSNative inited with apple id: " + APPLE_ID);
			_IsInited = true;
			_initIOSNative(APPLE_ID);
